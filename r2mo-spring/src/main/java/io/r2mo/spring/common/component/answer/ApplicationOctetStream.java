 ...
 */
class ApplicationOctetStream implements Reply {

    @Override
    public boolean write(final Binary binary, final HttpServletResponse response) {
        // 设置 Zip 相关的响应头
        response.setContentType(MIME.APPLICATION_OCTET_STREAM);
        response.setHeader(HttpHeaders.CONTENT_TYPE, MIME.APPLICATION_OCTET_STREAM);
