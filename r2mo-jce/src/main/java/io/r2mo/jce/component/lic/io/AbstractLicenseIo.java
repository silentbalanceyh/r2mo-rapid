...
import io.r2mo.jce.component.lic.domain.LicenseID;
...
abstract class AbstractLicenseIo implements LicenseIo {
    ...
    // 许可证文件路径
    protected String nameLic(final LicenseID licenseFile,
    ...
    // 签名文件路径
    protected String nameSig(final LicenseID licenseFile,
                             final LicenseConfiguration configuration) {
        return this.nameLicense(licenseFile, configuration) + ".sig";
    }

    // 密钥文件路径（公钥）
    protected String nameKey(final LicenseID licenseFile,
                             final LicenseConfiguration configuration) {
