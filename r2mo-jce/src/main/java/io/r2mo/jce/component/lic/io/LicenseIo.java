...
public interface LicenseIo {
     ...
     * <pre>
     *     流程：
     *     1. 根据实际内容计算 {@link LicenseLocation} 相关位置信息
     *        - 私钥位置 {@link LicenseLocation#ioPrivate()}
     ...
     *     4. 将数据写入到指定位置 {@link LicenseLocation#ioLicenseDirectory()}，文件名使用
     *        - {@link LicenseLocation#ioAlgorithm()} + {@link LicenseFile#format()}
     *     5. 在指定位置打包 *.zip 文件，然后将此文件 *.zip 转换成 {@link InputStream}
     * </pre>
     *
     * @param fileData License 文件对象
     * @param location 路径对象
     *
     * @return 返回 zip 压缩之后的 InputStream 数据流
     */
    InputStream writeTo(LicenseLocation location, LicenseFile fileData);
     ...
     *     2. 根据 LicenseId 计算 {@link LicenseLocation} 相关位置信息
     *        - 获取服务端存储的 *.sig 文件
     *        - 公钥位置 {@link LicenseLocation#ioPublic()}
     ...
     */
    LicenseData verify(LicenseLocation location, LicenseFile fileData);
}
