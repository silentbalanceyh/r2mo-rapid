...
import io.r2mo.spring.security.extension.cache.CacheOfFactorySecurity;
 ...
 *        - 这种模式自定义化的程度更高
 *     2. 直接定义类继承 {@link CacheOfFactorySecurity} 并重写缓存实现方案
...
public class AuthUserCache implements UserCache {
    private static final Cc<String, CacheOfFactory> CC_FACTORY = Cc.openThread();

    private CacheOfFactory factory() {
        return CC_FACTORY.pick(() -> {
            final CacheOfFactory found = SPI.findOneOf(CacheOfFactory.class);
